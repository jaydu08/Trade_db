...
→ 逐个查询 ak.futures_zh_spot(market="CF")
→ 计算相对昨结算价的正向涨跌幅，且 ≥ 2.0%
→ 检查该品种 24 小时冷却去重倒计时
→ 线程池提交 _analyze_and_push：
    ├── 聚合该品种最新资讯
    ├── LLM 结构化商品归因（CommodityAttribution）
    ├── Path A：本地 ChromaDB 产业链映射
    ├── Path B：LLM生成靶向搜索词 + 全网实时找寻受益股
    ├── LLM 合成具体受益股票代码（TradingIdea）
    └── 组装为【单条完整中文消息】
→ Notifier.broadcast() 一次性发送完整策略报告
```
